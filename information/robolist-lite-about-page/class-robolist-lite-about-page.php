<?php
/**
 * Robolist Lite - About page class
 * @package Robolist Lite
 * @subpackage Admin
 * @since 1.0.0
 */
if ( ! class_exists( 'robolist_lite_About_Page' ) ) {

	// Include utils functions
	/**
	 * Singleton class used for generating the about page of the theme.
	 */
	class Robolist_Lite_About_Page {
		/**
		 * Define the version of the class.
		 *
		 * @var string $version The Robolist_Lite_About_Page class version.
		 */
		...
		/**
		 * The single instance of Robolist_Lite_About_Page
		 *
		 * @var Robolist_Lite_About_Page $instance The  Robolist_Lite_About_Page instance.
		 */
		private static $instance;

		/**
		 * The Main Robolist_Lite_About_Page instance.
		 *
		 * We make sure that only one instance of Robolist_Lite_About_Page exists in the memory at one time.
		 *
		 * @param array $config The configuration array.
		 */
		public static function init( $config ) {
			if ( ! isset( self::$instance ) && ! ( self::$instance instanceof Robolist_Lite_About_Page ) ) {
				self::$instance = new Robolist_Lite_About_Page;
			...
			if ( ! empty( $this->menu_name ) && ! empty( $this->page_name ) ) {
				...
				add_theme_page(
					$this->menu_name, $title, 'activate_plugins', $this->theme_slug . '-welcome', array(
						$this,
						'robolist_lite_about_page_render',
					)
				);
			}
		}
		...
		public function activation_admin_notice() {
			...
			if ( is_admin() && ( 'themes.php' == $pagenow ) && isset( $_GET['activated'] ) ) {
				add_action( 'admin_notices', array( $this, 'robolist_lite_about_page_welcome_admin_notice' ), 99 );
			}
		}
		 ...
		 */
		public function robolist_lite_about_page_welcome_admin_notice() {
		 ...
		 */
		public function robolist_lite_about_page_render() {
			...
			if ( ! empty( $welcome_title ) || ! empty( $welcome_content ) || ! empty( $this->tabs ) ) {
				...
				if ( ! empty( $welcome_title ) ) {
					...
					echo '<img src="'.esc_url(get_template_directory_uri().'/information/robolist-lite-about-page/images/codethemes-logo-dark.png').'"></h1>';
				}


				if ( ! empty( $welcome_content ) ) {
					echo '<div class="about-text-wrap"> <div class = "about-img"> <img src="'.esc_url(get_template_directory_uri().'/screenshot.png').'" height="100px" width="100px"></h1> </div><div class="about-content-wrap">' . wp_kses_post( $welcome_content ) . '</div></div>';
				}
...
//
		...
		public function get_plugin_icon( $arr ) {
			...
			if ( ! empty( $arr['svg'] ) ) {
				...
				$plugin_icon_url = get_template_directory_uri() . '/information/acp-notifications/acp-about-page/images/placeholder_plugin.png';
			}
					...
					foreach ( $actions as $action_key => $action_value ) {
						...
						if ( ! empty( $action_value['plugin_slug'] ) ) {
							...
							if($network=='local' && $class=='install-now button') {
                                ?>
                                <p><?php echo esc_html__('If plugin installation failed then click on the&nbsp;','robolist-lite'); ?><a
                                            href="<?php echo esc_url(admin_url('themes.php?page=tgmpa-install-plugins')); ?>"
                                            target="_blank"><?php echo esc_html__('link.', 'robolist-lite'); ?></a>
                                </p>
                                <?php
                            }
                                ?>
        ...
        public function demo_import() {
            echo '<div class="feature-section three-col">';
            echo '<div class="feature-section acp-demo-import"><img src="'.esc_url(get_template_directory_uri().'/screenshot.png').'" width="300px"><h4>';
            printf(
                wp_kses(
                /* translators: 1: link to WP admin new post page. */
                    __( 'Need To Import Demo&nbsp;<a href="%1$s" class="button button-primary">Click Here</a>.', 'robolist-lite' ),
                    array(
                        'a' => array(
                            'href' => array(),
                            'class' => array(),
		...
		public function style_and_scripts( $hook_suffix ) {
			...
			wp_enqueue_style( 'robolist-lite-about-page-css', get_template_directory_uri() . '/information/robolist-lite-about-page/css/rl_about_page_css.css', array(), ROBOLIST_VERSION );

			if ( 'appearance_page_' . $this->theme_slug . '-welcome' == $hook_suffix ) {

				wp_enqueue_script( 'robolist-lite-about-page-js', get_template_directory_uri() . '/information/robolist-lite-about-page/js/rl_about_page_scripts.js', array( 'jquery' ), ROBOLIST_VERSION );
				...
				wp_localize_script(
					'acp-about-page-js', 'tiAboutPageObject', array(
						...
						'activating_string'   => esc_html__( 'Activating', 'robolist-lite' ),
					)
				);
